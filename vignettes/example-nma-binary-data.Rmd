...
Assume new treatment is "SUN" and is to be compared vs all other treatments.

__Table__ Odds ratios SUN vs other treatments
```{r}
OR <- get_mtc_newVsAll(model, new.lab = "SUN", transform = "exp", digits = 2)
OR
```

__Table__ Probability SUN better than other treatments (better meaning larger OR)
```{r}
get_mtc_probBetter(model, new.lab = "SUN", smaller.is.better = FALSE, sort.by = "effect")
```

__Figure__ Forest plot SUN vs other treatments
```{r, results='asis', fig.height=3, fig.width=6, warning=FALSE}
plot_mtc_forest(x = OR, 
                lab = "Odds ratio SUN vs others",
...
# Forest plot
```{r}
HR_i    <- get_mtc_newVsAll(model, new.lab = "PAZO", transform = "exp", digits = 2)
